  ...
  useEffect(() => {
    const fetchGroups = async () => {
      try {
        ...
        setSessionsNo(statsRes.data.upcomingSessions); 
        setResourcesNo(statsRes.data.resourcesShared);
        setTotalTodos(statsRes.data.totalTodos);
      } catch (err) {
    ...
    <div className="flex min-h-screen bg-gray-50 dashboard-wrapper">
      ...
      <div className="flex-1 px-10 py-8 dashboard-main">
        <div className="mb-8 flex justify-between dashboard-header">
          ...
          <img src={userData.avatar || "https://ui-avatars.com/api/?name=" + firstName} className="h-15 rounded-full" />
        </div>
          ...
          {groupData.length > 0 ? (
            <div className="grid grid-cols-4 gap-6 group-grid">
              {groupData.map((group, i) => (
                <Link to={"/group/" + group._id} key={i} className="group">
                  <div className="bg-white rounded-xl shadow hover:scale-105 transition p-4 flex flex-col ">
                    <img
                      src={group.imageURL || "https://picsum.photos/300/200"}
                      alt={group.groupName}
